...
public class RegisterUserUseCase 
{
    ...
    public RegisterUserUseCase(IUserRepository userRepository, IJwtFactory jwtFactory)
    {
        _userRepository = userRepository;
        _jwtFactory = jwtFactory;
    }

    public async Task<ResultResponse<AccessTokenResponse>> HandleAsync(RegisterUserRequest request)
    {
        if (await _userRepository.AnyEmailAsync(request.Email))
        {
            return new () { StatusCode = HttpStatusCode.Conflict, Message = "Email already exists" };
        }

        if (await _userRepository.AnyUsernameAsync(request.Username))
        {
            return new() { StatusCode = HttpStatusCode.Conflict, Message = "Username already exists" };
        }

        var result = await _userRepository.CreateAsync(request.Username, request.Email, request.Password);
        ...
        if (!result.Succeeded)
        {
            return new ()
            {
                ...
                Message = string.Join(" | ", result.Errors.Select(static error => error.Description))
            };
        }
        ...
        return new () { StatusCode = HttpStatusCode.Created, Data = response };
    }
}